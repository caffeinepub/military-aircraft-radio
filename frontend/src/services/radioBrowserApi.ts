...
export async function fetchCountries(): Promise<{ name: string; stationcount: number }[]> {
  const res = await apiFetch('/json/countries?order=stationcount&reverse=true&limit=100');
...
export async function fetchTags(): Promise<{ name: string; stationcount: number }[]> {
  const res = await apiFetch('/json/tags?order=stationcount&reverse=true&limit=50&hidebroken=true');
  if (!res || !res.ok) return [];
  try {
    return res.json();
  } catch {
